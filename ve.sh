...
time=$(date +%s)
giorni=$((($(date +%s)-$(date +%s --date "2018-12-22"))/(3600*24)))
persone="49"
ora=$(date +"%k")
...
- 1 #SEAWATCH 3, temperatura '"$airTemperatureSW"' °C, onda '"$waveHeightSW"' '"$urlSW"'
- 2 "PROF ALBRECHT PENCK", temperatura '"$airTemperaturePAP"' °C, onda '"$waveHeightPAP"' '"$urlPAP"'
